...
inline constexpr units::meter_t driveBaseRadius =
    units::math::hypot(trackWidth / 2.0, wheelBase / 2.0);
...
static const auto [frontLeftZeroRotation, frontRightZeroRtoation,
                   backLeftZeroRotation, backRightZeroRotation] =
    []() -> std::tuple<frc::Rotation2d, frc::Rotation2d, frc::Rotation2d,
                       frc::Rotation2d> {
  switch (Constants::getRobot()) {
  case RobotType::ROBOT_2026_COMP:
    return {frc::Rotation2d(units::radian_t(0.126 + M_PI)),
            {units::radian_t(0.759 + 0.03)},
            {units::radian_t(-1.630)},
            {units::radian_t(-2.072)}};
  default:
    return {};
  };
}();
...
inline constexpr ctre::phoenix6::swerve::ClosedLoopOutputType
...
inline const units::radian_t turnPIDMinInput{-M_PI};
inline const units::radian_t turnPIDMaxInput{M_PI};
...
inline const pathplanner::RobotConfig ppConfig{
    robotMass, robotMOI,
    pathplanner::ModuleConfig(wheelRadius, maxSpeed, wheelCOF, driveGearbox,
                              driveMotorReduction, driveMotorCurrentLimit, 1),
    moduleTranslations};
